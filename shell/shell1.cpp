...
void Shell::myod(vector<string>& s) {
    pid_t pid;
    bool background = false; // 标记是否在后台运行
    size_t size = s.size();
    if (size > 0 && s[size - 1] == "&") {
        background = true;
        s.pop_back(); // 移除 "&" 符号
    ...
    if (pid == 0) {
        ...
        for (size_t i = 0; i < s.size(); ++i) {
            if (s[i] == ">") {
                // 输出重定向：覆盖写入文件
                int fd = open(s[i + 1].c_str(), O_WRONLY | O_CREAT | O_TRUNC, 0644);
                if (fd == -1) {
                    perror("open()");
                    exit(1);
                }
                ...
                // 输出重定向：追加写入文件
                int fd = open(s[i + 1].c_str(), O_WRONLY | O_CREAT | O_APPEND, 0644);
                if (fd == -1) {
                    perror("open()");
                    exit(1);
                }
                ...
                // 输入重定向
                int fd = open(s[i + 1].c_str(), O_RDONLY);
                if (fd == -1) {
                    perror("open()");
                    exit(1);
                }
                ...
                args.push_back(const_cast<char*>(s[i].c_str()));
            }
        }
...
void Shell::mycd(vector<string> &s)
{
    ...
    {
        const char* homeDir = getenv("HOME");//getenv用于获取环境变量的指针，参数指定要获取的环境变量的名称，
        //找到返回该环境变量的指针，如果没有找到则返回空指针
        if (homeDir)
        {
            chdir(homeDir);
        } 
    ...
    else
    {
        ...
        if (chdir(path) == -1) 
        {
            std::cout << "cd: 无法进入目录 '" << path << "'" << std::endl;
        }
    }
}
...
void Shell::parse(string &s)
{
    ...
    for(vector<vector<string>>::iterator it=tokens.begin();it!=tokens.end();it++)
    {
        vector<string>:: iterator is= (*it).begin();
        if((*is).compare("cd")==0)
        {
            mycd(*it);
        }
        else if((*is).compare("ls")==0||(*is).compare("cat")==0 ||(*is).find("./")!=string::npos)
        {
            myod(*it);
        }
        else if((*is).compare("rm")==0||(*is).compare("mkdir")==0||(*is).compare("wc")==0)
        {
            myod(*it);
        }
        else if((*is).compare("grep")==0)
        {
            mygrep(*it);
        }
        else if((*is).compare("echo")==0)
        {
            myod(*it);
        }
        else
        {
            cout << "zsh: command not found: " << (*is) << endl;
        }
    }
}
