    plot_spiral_and_intersections(320.15948,387.571286)
    ...
    % 绘制所有交点之间的连线及编号
    num_intersections = length(x_max_i);

    % 连接每对交点
    for i = 1:num_intersections - 1
        % 连接每对交点
        plot([x_max_i(i), x_max_i(i+1)], [y_max_i(i), y_max_i(i+1)], 'k--', 'LineWidth', 1.5);

        % 计算连线中点位置
        mid_x = (x_max_i(i) + x_max_i(i+1)) / 2;
        mid_y = (y_max_i(i) + y_max_i(i+1)) / 2;

        % 添加连线编号
        text(mid_x, mid_y, sprintf('%d', num_intersections + i), 'FontSize', 12, 'HorizontalAlignment', 'center', 'BackgroundColor', 'w');
    end
