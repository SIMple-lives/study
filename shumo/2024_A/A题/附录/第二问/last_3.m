...
% 从第301点开始选择
start_index = 301; % 设置起始点索引
num_plot_points = num_points - start_index + 1; % 从第301点开始绘制所有剩余点
target_distances = target_distances(start_index:end); % 只选择301及以后的点

% 存储点的坐标
coordinates = zeros(num_plot_points, 2); % 用于存储坐标 (x, y)
...
% 将坐标写入 Excel 文件
filename = 'spiral_points.xlsx'; % Excel 文件名
writematrix(coordinates, filename, 'Sheet', 1, 'Range', 'A1'); % 将坐标保存到 Excel 中
fprintf('坐标已保存到文件: %s\n', filename);
